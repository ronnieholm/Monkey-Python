...
class Evaluator:
    ...
    def _extend_function_environment(self, function: monkey_object.Function, args: List[monkey_object.MonkeyObject]) -> environment.Environment:
    ...
    def _eval_program(self, stmts: List[ast.BlockStatement], env: environment.Environment) -> Optional[monkey_object.MonkeyObject]:
        result = None
        for stmt in stmts:
    ...
    def _eval_block_statement(self, stmts: List[ast.Statement], env: environment.Environment) -> Optional[monkey_object.MonkeyObject]:
        result = None
        for stmt in stmts:
            result = self.eval(stmt, env)
    ...
    def _eval_prefix_expression(self, operator: str, right: monkey_object.MonkeyObject) -> monkey_object.MonkeyObject:
        if operator == "!":
    ...
    def _eval_bang_operator_expression(self, right: monkey_object.MonkeyObject) -> monkey_object.MonkeyObject:
    ...
    def _eval_infix_expression(self, operator: str, left: monkey_object.MonkeyObject, right: monkey_object.MonkeyObject) -> monkey_object.MonkeyObject:
        if left.type_() == monkey_object.ObjectType.INTEGER and right.type_() == monkey_object.ObjectType.INTEGER:
            return self._eval_integer_infix_expression(operator, left, right)
        if left.type_() == monkey_object.ObjectType.STRING and right.type_() == monkey_object.ObjectType.STRING:
        ...
        if left.type_() != right.type_():
            return monkey_object.Error(f"type mismatch: {left.type_().value} {operator} {right.type_().value}")
    ...
    def _eval_index_expression(self, left: monkey_object.MonkeyObject, index: monkey_object.MonkeyObject) -> monkey_object.MonkeyObject:
        if left.type_() == monkey_object.ObjectType.ARRAY and index.type_() == monkey_object.ObjectType.INTEGER:
    ...
    def _eval_hash_index_expression(self, expr: monkey_object.MonkeyObject, index: monkey_object.MonkeyObject) -> monkey_object.MonkeyObject:
